package org.example.scraper.scraper;
...
public class HtmlPPKScraper {
    ...
    public static String extractTeacherBlock(String html) {
        ...
        if (!matcher.find()) {
            throw new ScraperHtmlValidationException("'teachers' block not found");
        }
    ...
    public static String extractStudentsBlock(String html) {
        ...
        Matcher matcher = pattern.matcher(html);
        if (!matcher.find()) {
            throw new ScraperHtmlValidationException("'Students' block not found");
        }
        return matcher.group(1);
    }

    public static List<ScheduleEntry> parseSchedule(String studentsBlock, String groupName) throws ScraperHtmlValidationException {
        ...
        if (!groupFound) {
            throw new ScraperHtmlValidationException("Group '" + groupName + "' not found");
        }
